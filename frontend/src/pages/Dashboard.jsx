...
export default function Dashboard({ adminMode }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-2">
            {adminMode ? '管理员仪表盘' : '今日概览'}
            <span className={`text-xs px-3 py-1 rounded-full ${adminMode ? 'bg-purple-100 text-purple-700' : 'bg-emerald-100 text-emerald-700'}`}>
              {adminMode ? 'ADMIN' : 'USER'} MODE
            </span>
          </h1>
        ...
        <Link
          to="/focus"
          className="inline-flex items-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-sky-500 text-white shadow-lg"
        >
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <StatsCard
          ...
          accent={adminMode ? 'purple' : 'emerald'}
        />
        <StatsCard
          title="今日专注"
